...
\begin{abstract}
  Common phases of mass transfer in \mr{massive} stellar binaries are
  case A (during the donor's main sequence) and case B (after the
  donor's main sequence but before helium core depletion). Most stars
  see stellar radii significantly grow after the main sequence, making
...
(case B) \mr{phase}, which together account for $\sim99\%$ of the donor's
lifetime \citep{kippenhahn:67}.
...
rate of case A \citep{demink:08}, affecting the evolution of the star and consequently
potentially altering stellar parameters that determine the rate of
...
\citep{anders:23, johnston:24}. \mr{Here,} we explore the radial
evolution of very massive stars \mr{and its impact on when
  mass-transfer starts. We also show variations in} convective
boundary mixing and metallicity, \mr{and compare to stellar} models
...
% \citep{stevenson:17, vignagomez:18, riley:22}. \mr{Stars with masses
%   $\geq50\,M_\odot$ in \textsc{SSE}/\textsc{COMPAS} are the result of
%   extrapolation of the fits to the models of \cite{pols:98}}.

When using overshooting \mr{(top)}, our
...
\cite{claret:18}. \mr{We compare} this relatively ``large
overshooting'' \mr{with models} not including any convective boundary
mixing \mr{(bottom)}, and to the \cite{pols:98} models including an
effectively mass-dependent overshooting (bottom panel). \mr{These are
  the stellar models underpinning \textsc{SSE} \citep{hurley:00},
  which we generated using \textsc{COMPAS} \citep{stevenson:17,
    vignagomez:18, riley:22}. We note that stars with masses
  $\geq50\,M_\odot$ in \textsc{SSE}/\textsc{COMPAS} are the result of
  extrapolation of the fits to the models of \cite{pols:98}}.

The red and blue dashed lines
in each panel of \Figref{fig:radii} denote the maximum radius during
the main sequence and helium core burning phase, respectively. These
mark the maximum Roche radius for a case A and case B donor,
respectively. The right axis shows orbital separations $a$ where the
stellar radius meets the roche radius computed from
